...
$page_values = array(
    'root'              => ((isset($forum['forumid']) && $forum['parentid']==0) ? $MOD_FORUM['TXT_READONLY_B'] : ''),
...
?>
<p></p>
<h3><?php echo $MOD_FORUM['TXT_POSTINGS_B'] ?></h3>
...
<?php
    ...
    <li class='{{ class }}'>
        <div class='forum_list_action'>
            ...
            <a href='#' style=\"{{ display }}\" onclick=\"delete_thread('forum_".$section_id."',{{ id }},'{{ title }}','{{ class }}','{{ lang }}');\"><img class='f_action' src='".THEME_URL."/images/delete_16.png' alt='' title='delete'></a>
        </div>
    ...
    $t = array(
        '{{ class }}'   => "thread",
        '{{ lang }}'    => LANGUAGE,
        '[ {{ id }} ]'  => '<strong>'.'ThreadID'.'</strong>',
        '{{ date }}'    => '<strong>'.$MOD_FORUM['TXT_DATE_B'].'</strong>',
        '{{ title }}'   => '<strong>'.$MOD_FORUM['TXT_TITLE_B'].'</strong>',
